...
MODERATOR_LEVELS = {"Country Lead", "Organizer", "Admin"}
CERTIFICATE_LEVEL_KEYS = {
    "Ambassador": "level.ambassador",
    "Active Ambassador": "level.ambassador",
    "Senior Ambassador": "level.ambassador",
    "Country Lead": "level.coordinator",
    "Organizer": "level.coordinator",
    "Admin": "level.coordinator",
}


def L(key: str, text: str) -> str:
    return f'<strong data-i18n="{escape(key)}">{escape(text)}</strong>'
ADMIN_LEVELS = {"Admin"}
app = FastAPI(
    title="Generation Justice",
    description="Membership hub with applications, certificates, discussions, courses, publications, and internships.",
    version="3.0.0",
)
...
def seed_data(connection: sqlite3.Connection) -> None:
    connection.execute(
        """
        UPDATE users
        SET membership = 'Ambassador'
        WHERE membership NOT IN ('Ambassador', 'Active Ambassador', 'Senior Ambassador', 'Country Lead', 'Organizer', 'Admin')
        """
    )
    ...
    if table_is_empty(connection, "comments"):
        connection.executemany(
            ...
            [
                ...
                ("Marcus Lee", "The member hub makes it easier for our club to share campaign updates.", "Apr 22, 2026 at 09:20"),
            ],
        )
...
def render_simple_cards(
    items: list[dict],
    fields: list[tuple[str, str, str]],
    class_name: str = "resource-card",
) -> str:
    cards = []
    for item in items:
        body = "".join(
            f"<p>{L(i18n_key, label)}: {escape(str(item[key]))}</p>"
            for key, i18n_key, label in fields
            if item.get(key)
        )
...
def render_comments(comments: list[dict], user: dict | None = None) -> str:
    ...
    return "\n".join(
        ...
        <article class="comment" data-comment-id="{comment["id"]}">
            ...
            {f'<button class="danger-button delete-comment" type="button" data-comment-id="{comment["id"]}" data-i18n="comments.delete">Delete Comment</button>' if can_delete else delete_button}
        </article>
        """
            ...
            <article class="discussion-topic" data-topic-id="{topic["id"]}">
                <div class="topic-head">
                    <div>
                        ...
                        <small><span data-i18n="dyn.startedBy">Started by</span> {escape(topic["created_by"])} - {escape(topic["created_at"])}</small>
                    </div>
                </div>
...
def render_lessons(lessons: list[dict]) -> str:
    return "\n".join(
        ...
        <article class="resource-card">
            ...
            <p>{L("label.date", "Date:")} {escape(lesson["lesson_date"])}</p>
            <p>{L("label.host", "Host:")} {escape(lesson["host"])}</p>
            <a class="button" href="{escape(lesson["link"])}" target="_blank" rel="noopener" data-i18n="lessons.join">Join Live Lesson</a>
        </article>
        """
...
def render_magazines(magazines: list[dict]) -> str:
    return "\n".join(
        ...
        <article class="resource-card">
            ...
            <a class="button secondary" href="{escape(magazine["link"])}" data-i18n="pub.openMag">Open Magazine</a>
        </article>
        """
...
def render_panels(panels: list[dict]) -> str:
    return "\n".join(
        ...
        <article class="resource-card">
            ...
            <p>{L("label.date", "Date:")} {escape(panel["panel_date"])}</p>
            <p>{L("label.speakers", "Speakers:")} {escape(panel["speakers"])}</p>
            <a class="button" href="{escape(panel["link"])}" target="_blank" rel="noopener" data-i18n="panels.join">Join Panel</a>
        </article>
        """
        for panel in panels
    )
...
def render_projects(projects: list[dict]) -> str:
    return "\n".join(
        ...
        <article class="resource-card">
            ...
            <p>{L("label.coordinator", "Coordinator:")} {escape(project["coordinator"])}</p>
            <p>{L("label.availability", "Availability:")} {escape(project["spots"])}</p>
        </article>
        """
        for project in projects
    )
...
def render_winners(winners: list[dict]) -> str:
    return "\n".join(
        ...
        <article class="resource-card winner-card">
            ...
            <p>{L("label.winner", "Winner:")} {escape(winner["winner_name"])} ({escape(winner["country"])})</p>
...
def render_user_list(items: list[dict], empty_key: str, empty_text: str) -> str:
    if not items:
        return f'<p class="muted" data-i18n="{escape(empty_key)}">{escape(empty_text)}</p>'
    return "\n".join(
        f"""
...
def replace_content_placeholders(content: str, context: dict, user: dict | None) -> str:
    ...
    replacements = {
        ...
        "__COURSES__": render_simple_cards(
            context.get("courses", []),
            [
                ("category", "label.category", "Category"),
                ("title", "label.course", "Course"),
                ("duration", "label.duration", "Duration"),
                ("summary", "label.summary", "Summary"),
            ],
        ),
        "__MAGAZINES__": render_magazines(context.get("magazines", [])),
        "__BOOKS__": render_simple_cards(
            context.get("books", []),
            [
                ("year", "label.year", "Year"),
                ("title", "label.book", "Book"),
                ("summary", "label.summary", "Summary"),
            ],
        ),
        "__PANELS__": render_panels(context.get("panels", [])),
        "__USER_ESSAYS__": render_user_list(submissions["essays"], "empty.noEssays", "No essay submissions yet."),
        "__USER_ARTICLES__": render_user_list(submissions["articles"], "empty.noArticles", "No article submissions yet."),
        "__USER_INTERNSHIPS__": render_user_list(
            submissions["internships"], "empty.noInternships", "No internship applications yet."
        ),
        ...
        "__CERTIFICATE_LEVEL_KEY__": escape(
            CERTIFICATE_LEVEL_KEYS.get(user["membership"], "level.member") if user else "level.member"
        ),
    }
...
def render_page(request: Request, template_name: str, active_page: str, title: str, protected: bool = False, **context):
    ...
    if user:
        level_key = CERTIFICATE_LEVEL_KEYS.get(user["membership"], "level.member")
        chip_text = (
            f'<span data-i18n="{escape(level_key)}">{escape(user["membership"])}</span> '
            f'<span data-i18n="member.chipSuffix">Member</span>'
        )
    else:
        chip_text = ""
...
@app.get("/membership/ambassador")
def membership_ambassador(request: Request):
    return render_page(
        request,
        "membership_ambassador.html",
        "membership",
        "Generation Justice | Ambassador Application",
    )
...
def apply_for_membership(payload: ApplicationPayload):
    ...
    membership = clean(payload.membership, 80) or "Ambassador"
    if membership not in MEMBERSHIP_LEVELS:
